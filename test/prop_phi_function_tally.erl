...
%%% phi(T1, T2, P0, P+, P-)
%%phi_v0(T1, T2, P) -> phi_v0(T1, T2, P, ty_rec:empty(), ty_rec:any(), sets:new(), sets:new()).
%%phi_v0(T1, T2, [], D , C, Fixed, M) ->
%%  % (T1 <: D) or (C <: T2)
%%  constraint_set:join(
%%    ty_rec:normalize(ty_rec:diff(T1, D), Fixed, M),
%%    ty_rec:normalize(ty_rec:diff(C, T2), Fixed, M)
%%  );
%%% phi(T1, T2, P U {S1 --> S2}, D, C)
%%phi_v0(T1, T2, [Function | P], D, C, Fixed, M) ->
%%  S1 = ty_function:domain(Function),
%%  S2 = ty_function:codomain(Function),
%%  constraint_set:meet(
%%    phi_v0(T1, T2, P, D, ty_rec:intersect(C, S2), Fixed, M),
%%    phi_v0(T1, T2, P, ty_rec:union(D, S1), C, Fixed, M)
%%  ).
%%
%%
...
prop_phi_no_timeout() ->
  ?FORALL({T1, T2, P}, {limited_ast(), limited_ast(), list({limited_ast(), limited_ast()})},
    begin
      PFuns = lists:map(fun({D, C}) -> ty_function:function(D, C) end, lists:sublist(P, 3)),
      persistent_term:put(phi_norm_fun, v0),
      {Ti1, _} = timer:tc(fun() -> dnf_ty_function:explore_function_norm_v0(T1, T2, PFuns, ty_rec:empty(), ty_rec:any(), sets:new(), sets:new()) end),

      persistent_term:put(phi_norm_fun, v),
      {Ti2, _} = timer:tc(fun() -> dnf_ty_function:explore_function_norm(T1, ty_rec:negate(T2), PFuns, sets:new(), sets:new()) end),
      io:format(user, "Res: ~p vs ~p~n", [Ti1, Ti2]),
      true
    end
  ).
