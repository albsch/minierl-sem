...
-spec normalize_api(constraint:simple_constraints(), sets:new()) -> any() | {fail, norm}.
normalize_api(Constraints, FixedVariables) ->
  ToMeetNormalizedConstraints =
    lists:foldl(fun (Cs, Ac) -> normalize_constraints_api(Cs, Ac, FixedVariables) end, [[]], Constraints),

  error("Todo").
%%  SaturatedConstraints = saturate(ToMeetNormalizedConstraints, [], FixedVariables),
%%  SaturatedConstraints.



normalize_constraints_api({csubty, _loc, S, T}, AllNormalized, Fix) ->
  SnT = ty_rec:intersect(S, ty_rec:negate(T)),

  ty_rec:normalize(SnT).

  % sorted list of merged variables => [{V, S, T}]
%%  Normalized = norm_and_merge(SnT, [], Fix),
%%%%  sanity(Normalized),
%%  M = merge_and_meet(Normalized, AllNormalized),
%%  M.
%%
%%
